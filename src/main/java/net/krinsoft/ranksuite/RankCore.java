...
public class RankCore extends JavaPlugin {
	...
	public FileConfiguration getDB() {
		if (db == null) {
			db = YamlConfiguration.loadConfiguration(new File(getDataFolder(),
					"players.db"));
		}
		return db;
	}
	...
	public RankedPlayer promote(final String name) {
		...
		if (player == null) {
			...
			player = new RankedPlayer(this, name, rank, minutes,
		...
		if (player.addTime()) {
			...
			if (next.getPromotionMessage() != null) {
				for (String msg : next.getPromotionMessage().split("\n")) {
					promoted.getPlayer().sendMessage(
	...
	public void checkRank(CommandSender sender, OfflinePlayer player) {
		if (sender.equals(player)
			...
			sender.sendMessage(ChatColor.AQUA + name + ChatColor.GREEN
					+ (sender.equals(player) ? " are" : " is")
					+ " in the rank: " + ChatColor.AQUA + p.getRank().getName());
			sender.sendMessage(ChatColor.AQUA + name + ChatColor.GREEN
					+ (sender.equals(player) ? " have" : " has")
					+ " played for " + ChatColor.AQUA
					+ format(p.getTimePlayed()) + ChatColor.GREEN + ".");
			if (p.getRank().getNextRank() != null) {
				...
				if (next != null) {
					sender.sendMessage(ChatColor.AQUA
							...
							+ " in "
							+ ChatColor.AQUA
							+ (format(next.getMinutesRequired()
									- p.getTimePlayed())) + ChatColor.GREEN
							+ ".");
				}
			}
		}
