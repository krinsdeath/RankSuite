...
public class RankCore extends JavaPlugin {
	...
	public void onEnable() {
		...
		// build the ranks
		for (String rank : getConfig().getConfigurationSection("ranks")
				.getKeys(false)) {
			ConfigurationSection section = getConfig().getConfigurationSection(
					"ranks." + rank);
			Rank rank1 = new Rank(rank, section.getString("next_rank"),
					section.getInt("time_played"), section.getString("message"));
		...
		if (file.exists()) {
			getLogger().info(
					"Using AutoRank's data.yml for current rankings...");
		...
		this.leaderTask = getServer().getScheduler().scheduleSyncRepeatingTask(
				this, new Runnable() {
					public void run() {
						buildLeaderboard();
					}
		...
		this.updateTask = getServer().getScheduler().scheduleSyncRepeatingTask(
				this, new Runnable() {
					public void run() {
		...
		this.loginTask = getServer().getScheduler().scheduleSyncRepeatingTask(
				this, new Runnable() {
					public void run() {
						if (logins.size() > 0) {
	...
	public FileConfiguration getPlayersDB() {
		if (players_db == null) {
			players_db = YamlConfiguration.loadConfiguration(new File(
					getDataFolder(), "players.db"));
		}
		return players_db;
	}
	...
	public void savePlayersDB() {
		try {
			...
			getLogger().warning(
					"An error occurred while saving the players database.");
		}
	}
	...
	/**
	 * Attempts to reload the plugin's configuration file and restart the plugin
	 * tasks
	 */
	public void reload() {
		getServer().getScheduler().cancelTasks(this);
		...
		getConfig()
				.options()
				.header("You should always have one group whose time required is 0 minutes.\n"
	...
	public void buildLeaderboard() {
		...
		for (String key : getUuidDB().getKeys(false)) {
			...
			leaders.put(name, getUuidDB().getInt(uuid.toString()));
		}
	 ...
	 * 
	 * @param page
	 *            The page to begin searching on
	 ...
	 * 
	 * @param name
	 *            The UUID of the player we're fetching
	 ...
	 * 
	 * @param name
	 *            The name of the rank we're fetching
	...
	public Rank getRank(int mins) {
		...
		for (Rank r : ranks.values()) {
			if (r.getMinutesRequired() <= mins
					&& r.getMinutesRequired() >= highest) {
	 ...
	 * 
	 * @param name
	 *            The name of the player who has just logged in
	 */
	...
	/**
	 * Checks whether the specified player is qualified for a promotion, and
	 * promotes them if they are
	 * 
	 * @param uuid
	...
	public RankedPlayer promote(final UUID uuid) {
		...
		if (promoted == null) {
			getLogger().warning(
					"Something went wrong... a fetched player was null!");
			return null;
		}
		...
		if (player == null) {
			...
			if (rank == null) {
				getLogger()
						.info("No matching rank was found! Check config.yml for invalid syntax.");
				rank = ranks.values().toArray(new Rank[ranks.size()])[0];
			}
			debug(name + " determined to be " + rank.getName() + " with "
					+ minutes + " minute(s) played.");
			boolean exempt = !promoted.isOnline()
					|| promoted.getPlayer().hasPermission("ranksuite.exempt");
			player = new RankedPlayer(this, name, uuid, rank, minutes,
					System.currentTimeMillis(), exempt);
		}
		if (player.addTime()) {
			// player is qualified for a promotion
			Rank last = player.getRank();
			Rank next = getRank(player.getRank().getNextRank());
			rank(promoted.getName() + " has advanced from " + last.getName()
					+ " to " + next.getName() + "!");
			for (String cmd : this.promotion) {
				...
				getServer()
						.dispatchCommand(getServer().getConsoleSender(), cmd);
			}
			...
			RankSuiteRankChangeEvent event = new RankSuiteRankChangeEvent(name,
					player.getRank().getName(), next.getName());
	...
	/**
	 * Checks for a promotion for the specified player and then destroys their
	 * currently loaded object
	 * 
	 * @param uuid
	 *            The UUID of the player who is retiring
	 */
	...
	/**
	 * Tests whether the sender is allowed to check the player's rank, and then
	 * displays information about the specified player.
	 * 
	 * @param sender
	 *            The sender who issued the command
	 * @param player
	 *            The player whose rank is being checked
	 */
	public void checkRank(CommandSender sender, OfflinePlayer player) {
		if (sender.equals(player)
			...
			message.append(ChatColor.AQUA).append(name).append(ChatColor.GREEN)
					.append(equal ? " are" : " is").append(" in the rank: ")
					.append(ChatColor.AQUA).append(p.getRank().getName())
					.append("\n");
			message.append(ChatColor.AQUA).append(name).append(ChatColor.GREEN)
					.append(equal ? " have" : " has").append(" played for ")
					.append(ChatColor.AQUA)
					.append(FancyParser.toFancyTime(p.getTimePlayed()))
			...
			message.append(ChatColor.AQUA)
					...
					.append(" ranked ")
					.append(ChatColor.AQUA)
					.append(FancyParser.toFancyOrdinal(getLeader(player
							.getName()) + 1)).append(ChatColor.GREEN)
					.append(".\n");
			if (p.getRank().getNextRank() != null) {
				...
				if (next != null) {
					message.append(ChatColor.AQUA)
							.append(name)
							.append(ChatColor.GREEN)
							.append(" will rank up to ")
							.append(ChatColor.AQUA)
							.append(next.getName())
							.append(ChatColor.GREEN)
							.append(" in ")
							.append(ChatColor.AQUA)
							.append(FancyParser.toFancyTime(next
									.getMinutesRequired() - p.getTimePlayed()))
							.append(ChatColor.GREEN).append(".\n");
				}
			}
	 ...
	 * 
	 * @param uuid
	 *            The UUID of the player being reset.
	 */
	...
	/**
	 * Uses the specified player name and rank name to reset a player to the
	 * default rank
	 * 
	 * @param uuid
	 *            The UUID of the player
	 * @param rank
	 *            The rank being removed from the player
	 */
	...
	/**
	 * Resets the specified player to the specified base rank and removes the
	 * 'rank' from the player.
	 * 
	 * @param uuid
	 *            The UUID of the player
	 * @param rank
	...
	public void reset(UUID uuid, String rank, String base) {
		...
		getServer().getPluginManager().callEvent(event);
		Plugin plg = this.getServer().getPluginManager()
				.getPlugin("bPermissions");
		if (plg != null) {
			getServer().dispatchCommand(getServer().getConsoleSender(),
					String.format("exec u:%s a:rmgroup v:%s", name, rank));
			getServer().dispatchCommand(getServer().getConsoleSender(),
					String.format("exec u:%s a:addgroup v:%s", name, base));
			return;
		}
		plg = this.getServer().getPluginManager().getPlugin("PermissionsEx");
		if (plg != null) {
			getServer().dispatchCommand(getServer().getConsoleSender(),
					String.format("pex user %s removegroup %s", name, rank));
			getServer().dispatchCommand(getServer().getConsoleSender(),
					String.format("pex user %s addgroup %s", name, base));
			return;
		}
		plg = this.getServer().getPluginManager()
				.getPlugin("PermissionsBukkit");
		if (plg != null) {
			getServer().dispatchCommand(
					getServer().getConsoleSender(),
					String.format("permissions player removegroup %s %s", name,
							rank));
			getServer().dispatchCommand(
					getServer().getConsoleSender(),
					String.format("permissions player addgroup %s %s", name,
							base));
			return;
		}
		...
		if (plg != null) {
			getServer().dispatchCommand(getServer().getConsoleSender(),
					String.format("pgs %s %s", name, base));
		}
	}
