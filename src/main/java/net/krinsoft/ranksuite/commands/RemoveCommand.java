...
public class RemoveCommand extends BaseCommand {
	...
	public RemoveCommand(RankCore plugin) {
		...
		setName("remove");
		addUsage("[user]", "[mins]",
				"Removes playtime from the specified user.");
		setRequiredArgs(2);
	...
	public void runCommand(CommandSender sender, List<String> args) {
		...
		try {
			...
			sender.sendMessage(ChatColor.RED + "Time removed must be a "
					+ ChatColor.GREEN + "positive number" + ChatColor.RED + ".");
			return;
		}
		if (mins <= 0) {
			sender.sendMessage(ChatColor.RED + "Time removed must be a "
		...
		sender.sendMessage(ChatColor.AQUA + String.valueOf(mins)
				+ ChatColor.GREEN + " minute" + (mins > 1 ? "s have" : " has")
				+ " been removed from " + ChatColor.AQUA + target.getName()
				+ ChatColor.GREEN + ".");
	}
}
