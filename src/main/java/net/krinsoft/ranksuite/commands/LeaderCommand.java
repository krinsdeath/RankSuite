	...
	public void runCommand(CommandSender sender, List<String> args) {
		...
		try {
			...
			sender.sendMessage(ChatColor.GREEN + "===" + ChatColor.GOLD
					+ " Rank Leaders " + ChatColor.GREEN + "===");
			for (Map.Entry<Integer, Leader> entry : map.entrySet()) {
				sender.sendMessage(String.format("%1$-" + 4 + "s",
						(entry.getKey() + 1))
						+ ChatColor.GREEN
						+ " | "
						+ ChatColor.GOLD
						+ String.format("%1$-" + 17 + "s", entry.getValue()
								.getName())
						+ ChatColor.GREEN
						+ " | "
						+ ChatColor.AQUA
						+ plugin.getRank(entry.getValue().getTimePlayed())
								.getName());
			}
		} catch (NumberFormatException e) {
			sender.sendMessage(ChatColor.RED + "The page must be a "
